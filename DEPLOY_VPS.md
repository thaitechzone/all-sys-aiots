...
```bash
cd ~
git clone <repo-url> all-sys-aiots
cd all-sys-aiots
cp .env.example .env
```
...
scp .env root@<VPS_IP>:~/all-sys-aiots/.env

# แล้ว set สิทธิ์บน VPS
ssh root@<VPS_IP> "chmod 600 ~/all-sys-aiots/.env"
```
...
for v in nodered_data n8n_data influxdb_data grafana_data homeassistant_config npm_data; do
  docker run --rm -v all-sys-aiots_$v:/data -v $(pwd):/backup alpine \
